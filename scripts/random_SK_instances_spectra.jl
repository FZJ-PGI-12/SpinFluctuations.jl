...
for (k, instance_name) in enumerate(instance_names[loop_var:loop_var+199])
    ...
    try
        h5read(folder_name * instance_name, @sprintf("spectra_T_final_%i_tau_final_%i/T_%0.5f/data", T_final, τ_final, T_diags[1] / T_final), spec_sum)
    catch
